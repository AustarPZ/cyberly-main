...
function createProgressRouter(progressService) {
  ...
  router.post('/api/progress/sync-initial-assessment', requireAuth, async (req, res, next) => {
    try {
      res.json(await progressService.syncLatestInitialAssessment(req.session.userId));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.get('/api/recommendations/current', requireAuth, async (req, res, next) => {
    try {
      res.json(await progressService.getCurrentRecommendation(req.session.userId));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/api/recommendations/:id/viewed', requireAuth, async (req, res, next) => {
    try {
      res.json(await progressService.markViewed(req.session.userId, req.params.id));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post('/api/recommendations/:id/completed', requireAuth, async (req, res, next) => {
    try {
      res.json(await progressService.markCompleted(req.session.userId, req.params.id));
    } catch (error) {
      next(error);
    }
  });
