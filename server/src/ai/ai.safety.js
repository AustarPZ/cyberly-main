...
const UNSAFE_INPUT_PATTERNS = [
  /\b(steal|steals|stealing|harvest|dump|exfiltrate)\b.{0,60}\b(password|credential|cookie|token|otp)\b/i,
  ...
  /\b(create|write|make|generate)\b.{0,50}\b(phishing|scam|fake banking)\b.{0,30}\b(sms|text|message)\b/i,
  /\b(phishing|scam|fake banking)\b.{0,30}\b(sms|text|message)\b.{0,50}\b(create|write|make|generate)\b/i,
  /\bmalicious\b.{0,30}\blink\b.{0,40}\b(legitimate|safe|trusted)\b/i,
  /\b(bypass|evade|circumvent)\b.{0,60}\b(login|log[- ]?in|authentication|account access)\b/i,
  /\b(bypass|evade|circumvent)\b.{0,60}\b(bank account security|account security)\b/i,
