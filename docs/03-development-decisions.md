...
- The Initial Cyber Wellness Assessment is a fixed versioned 12-question baseline, not generated by AI.
- Assessment scoring is deterministic and calculated only on the backend.
- Measured ability is separate from self-reported `learner_profiles.familiarity_level`.
- Assessment measured ability must not be inferred from age, age group, or education level.
