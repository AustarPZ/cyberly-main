...
test('one unfinished only has no family controls and delegates only its exact target', () => {
  ...
  fireEvent.click(screen.getByRole('button',{name:'Resume scenario: Parcel SMS'}));
...
test('duplicate titles have equal, distinct session labels and exact internal targets without IDs', () => {
  ...
  expect(buttons[0]).toHaveAccessibleName('Resume scenario: Parcel SMS Saved practice 1');
  expect(buttons[1]).toHaveAccessibleName('Resume scenario: Parcel SMS Saved practice 2');
...
test('missing title never falls back to raw slug',()=>{
  ...
  expect(screen.getByRole('button',{name:'Resume scenario: Saved practice'})).toBeVisible();
...
test.each(['unknown','error'])('unfinished owner %s is not empty confirmation',state=>{
  ...
  expect(screen.queryByRole('button',{name:/Resume scenario/})).toBeNull();
});
...
test('duplicate labels survive family switches and an earlier attempt leaving the inventory',()=>{
  ...
  expect(screen.getByRole('button',{name:'Resume scenario: Parcel SMS Saved practice 2'})).toBeVisible();
});
...
test.each([{scopeKey:'new-learner',revision:0},{scopeKey:stamp.scopeKey,revision:1}])('scope change resets manual choice and rejects stale actions %j',scope=>{
  ...
  expect(screen.queryByText('Pause and check.')).toBeNull();expect(screen.queryByRole('button',{name:/Resume scenario/})).toBeNull();
